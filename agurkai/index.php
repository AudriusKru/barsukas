...
function redirect() 
{
    header('Location: http://localhost/barsukas/agurkai/');
    die;
}
function redirectToAction($action, $id = 0) 
{
    if ($id) {
        header('Location: http://localhost/barsukas/agurkai/?action='.$action.'&id='.$id);
    }
    else {
        header('Location: http://localhost/barsukas/agurkai/?action='.$action);
    }
    die;
}
// redirectBack()  //
